...
export default function SignUp() {
  ...
  const { setAuth, initState: _initState, setInitState } = useAuthStore();
  ...
  const handleLoginByStack = useCallback(
    async (token: string) => {
      try {
        ...
        setAuth({ email: formData.email, ...data });
        setInitState('done'); // Skip onboarding after signup
        navigate('/');
      } catch (error: any) {
    ...
    [formData.email, navigate, setAuth, setInitState, setGeneralError, setIsLoading, t]
  );
